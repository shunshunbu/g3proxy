        ...
        loop {
            match obj {
                ...
                StreamInspection::TlsModern(tls) => {
                    let result = tls.intercept(&mut inspector).await;
                    obj = result?;
                ...
                StreamInspection::Ftp(ftp) => match ftp.intercept().await? {
                    Some(new_obj) => {
                        obj = new_obj;
                    }
                    None => break,
                },
                StreamInspection::FtpUploadData(ftp_upload) => {
                    ftp_upload.intercept().await?;
                    break;
                }
